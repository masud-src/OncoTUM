    ...
    *Methods*:
    ...
    def forward(self, x: Any) -> float:
        ...
        with torch.no_grad():
            if random() < self.p:
                x = x * uniform(0.9, 1.1)
                std_per_channel = torch.stack(list(torch.std(x[:, i][x[:, i] > 0].float()) for i in range(x.size(1))))
                if torch.cuda.device_count() >= 1:
                    tensor = torch.tensor(1e-4, dtype=torch.float16).cuda()
                else:
                    tensor = torch.tensor(1e-4, dtype=torch.float16).cpu()
                std_per_channel = torch.where((std_per_channel == 0.0), tensor, std_per_channel)
                noise = torch.stack([torch.normal(0, std * 0.1, size=x[0, 0].shape) for std in std_per_channel]).to(
                    x.device)
                x = x + noise
